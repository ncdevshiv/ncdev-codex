...
pub enum WireApi {
    ...
    #[default]
    Responses,
    /// The standard OpenAI Chat Completions API at `/v1/chat/completions`.
    Chat,
}
...
impl ModelProviderInfo {
    ...
    pub(crate) fn to_api_provider(
        ...
        // Only use the ChatGPT backend URL when the provider explicitly
        // requires OpenAI auth AND the current auth mode is ChatGPT.
        // Non-OpenAI providers (OpenRouter, custom OSS, etc.) must never
        // resolve to the ChatGPT backend even if the user is logged in.
...
pub fn built_in_model_providers() -> HashMap<String, ModelProviderInfo> {
    ...
    // Bundle first-party providers plus local OSS providers by default.
    // Users can still add or override providers via `model_providers` in config.toml.
    [
