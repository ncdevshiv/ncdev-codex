...
pub(crate) fn auth_provider_from_auth(
    ...
    // Only fall back to OpenAI/ChatGPT login auth when the provider
    // explicitly requires it. Non-OpenAI providers (OpenRouter, custom
    // OSS, etc.) should never receive ChatGPT credentials.
