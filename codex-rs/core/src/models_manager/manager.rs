...
impl ModelsManager {
    ...
    pub fn new(
        ...
        let remote_models = model_catalog
            ...
            .unwrap_or_else(|| Self::initial_models_for_provider(&provider));
        Self {
    ...
    fn initial_models_for_provider(provider: &ModelProviderInfo) -> Vec<ModelInfo> {
    ...
    pub async fn set_provider(&self, provider_id: String, provider: ModelProviderInfo) {
        *self.provider_id.write().await = provider_id;
        let initial_models = Self::initial_models_for_provider(&provider);
        *self.provider.write().await = provider;
        *self.remote_models.write().await = initial_models;
    ...
    async fn fetch_and_update_models(&self) -> CoreResult<()> {
        ...
        *self.etag.write().await = etag.clone();
        self.cache_manager
            .persist_cache(provider_id.as_str(), &models, etag, client_version)
            .await;
        Ok(())
    }
    ...
    async fn try_load_cache(&self) -> bool {
        ...
        let cache = match self
            .cache_manager
            .load_fresh(provider_id.as_str(), &client_version)
            .await
        {
            Some(cache) => cache,
            None => {
    ...
    pub(crate) fn with_provider_for_tests(
        ...
        Self {
            ...
            etag: RwLock::new(None),
            cache_manager,
            provider_id: RwLock::new("test-provider".to_string()),
            provider: RwLock::new(provider),
        }
    }
...
mod tests {
    ...
    async fn refresh_available_models_fetches_without_chatgpt_auth() {
        ...
        assert!(
            ...
            "remote refresh should include dynamically fetched models"
        );
        assert_eq!(
            models_mock.requests().len(),
            1,
            "missing chatgpt auth should not block /models requests"
        );
    }
