...
pub(crate) struct SessionSettingsUpdate {
    ...
    pub(crate) personality: Option<Personality>,
    pub(crate) app_server_client_name: Option<String>,
    /// Updated model provider (when the user switches providers at runtime).
    ...
    pub(crate) async fn update_settings(
        ...
        match state.session_configuration.apply(&updates) {
            Ok(updated) => {
                ...
                // If the provider changed, hot-swap it in the session-scoped ModelClient.
        ...
        let should_exit = async {
            match sub.op.clone() {
                ...
                Op::OverrideTurnContext {
                    ...
                    // Deserialize provider if present.
                    let provider_info: Option<ModelProviderInfo> =
                        provider.and_then(|v| serde_json::from_value(v).ok());
