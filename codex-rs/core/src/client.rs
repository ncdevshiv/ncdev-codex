...
struct ModelClientState {
    ...
    /// The active provider. Wrapped in `RwLock` so TUI provider switches
    /// can hot-swap it without restarting the session thread.
...
impl ModelClient {
    ...
    /// Hot-swap the provider for this session. This is called when the
    /// user switches providers via the TUI without restarting the thread.
...
impl ModelClientSession {
    ...
    async fn stream_responses_api(
        ...
        if let Some(path) = &*CODEX_RS_SSE_FIXTURE {
            ...
            let provider_idle_timeout = self
                .client
                .state
                .provider
                .read()
                .unwrap_or_else(std::sync::PoisonError::into_inner)
                .stream_idle_timeout();
            let stream = codex_api::stream_from_fixture(path, provider_idle_timeout)
                .map_err(map_api_error)?;
