...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum SelectionHotKey {
    CtrlA,
    CtrlP,
}

pub(crate) struct SelectionHotKeyAction {
    pub hotkey: SelectionHotKey,
    pub action: SelectionAction,
    pub dismiss_on_trigger: bool,
}
...
pub(crate) struct SelectionViewParams {
    ...
    /// Popup-specific hotkeys that should run before default navigation.
    pub custom_hotkeys: Vec<SelectionHotKeyAction>,
}
...
impl ListSelectionView {
    ...
    pub fn new(params: SelectionViewParams, app_event_tx: AppEventSender) -> Self {
        ...
        let mut s = Self {
            ...
            custom_hotkeys: params.custom_hotkeys,
        };
        s.apply_filter();
        s
    }
    ...
    fn is_ctrl_p(key_event: KeyEvent) -> bool {
        matches!(
            key_event,
            KeyEvent {
                code: KeyCode::Char('p'),
    ...
    fn hotkey_matches(key_event: KeyEvent, hotkey: SelectionHotKey) -> bool {
        match hotkey {
            SelectionHotKey::CtrlA => Self::is_ctrl_a(key_event),
            SelectionHotKey::CtrlP => Self::is_ctrl_p(key_event),
        }
    }

    fn handle_custom_hotkey(&mut self, key_event: KeyEvent) -> bool {
        for hotkey in &self.custom_hotkeys {
            if Self::hotkey_matches(key_event, hotkey.hotkey) {
                (hotkey.action)(&self.app_event_tx);
                if hotkey.dismiss_on_trigger {
                    self.complete = true;
                }
                return true;
            }
...
impl BottomPaneView for ListSelectionView {
    fn handle_key_event(&mut self, key_event: KeyEvent) {
        if self.handle_custom_hotkey(key_event) {
            return;
        }
