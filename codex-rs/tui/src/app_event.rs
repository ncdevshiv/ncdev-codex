...
pub(crate) enum AppEvent {
    ...
    /// Start the `/model` flow using the active provider.
    OpenModelPicker,

    /// Open the provider picker from the model flow.
    OpenModelProvidersPopup,

    /// Select an existing provider by id.
    SelectModelProvider {
        provider_id: String,
    },

    /// Prompt for an API key for the selected provider.
    PromptProviderApiKey {
        provider_id: String,
    },

    /// Save a provider API key entered by the user.
    SubmitProviderApiKey {
        provider_id: String,
        api_key: String,
    },

    /// Open the add-custom-provider flow.
    PromptAddCustomModelProvider,

    /// Continue add-custom-provider flow with a base URL.
    SubmitCustomProviderBaseUrl {
        base_url: String,
    },

    /// Finalize add-custom-provider flow with base URL and API key.
    SubmitCustomProviderApiKey {
        base_url: String,
        api_key: String,
    },

    /// User chose ChatGPT login for OpenAI provider auth.
    UseOpenAiLogin,
