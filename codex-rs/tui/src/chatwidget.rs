...
const MODEL_ALL_SELECTION_VIEW_ID: &str = "model-all-selection";
...
use crate::bottom_pane::SelectionHotKeyAction;
...
impl ChatWidget {
    ...
    fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd {
            ...
            SlashCommand::Model => {
                ...
                self.app_event_tx.send(AppEvent::OpenModelPicker);
            }
            SlashCommand::Fast => {
    ...
    pub(crate) fn open_model_popup_with_presets(&mut self, presets: Vec<ModelPreset>) {
        ...
        self.bottom_pane.show_selection_view(SelectionViewParams {
            ...
            items,
            header,
            custom_hotkeys: vec![SelectionHotKeyAction {
                hotkey: SelectionHotKey::CtrlA,
                action: Box::new(|tx| {
                    tx.send(AppEvent::OpenModelProvidersPopup);
                }),
                dismiss_on_trigger: true,
            }],
            ..Default::default()
        });
    }
    ...
    pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
        ...
        for preset in presets.into_iter() {
            ...
            let search_value = Some(format!(
                "{} {}",
                preset.model,
                preset.description.to_lowercase()
            ));
            items.push(SelectionItem {
        ...
        self.bottom_pane.show_selection_view(SelectionViewParams {
            view_id: Some(MODEL_ALL_SELECTION_VIEW_ID),
            footer_hint: Some(footer_hint),
            items,
            header,
            is_searchable: true,
    ...
    pub(crate) fn open_model_providers_popup(
        &mut self,
        providers: Vec<(String, String, bool)>,
        current_provider_id: &str,
    ) {
        let mut items = Vec::new();
        for (provider_id, provider_name, requires_openai_auth) in providers {
            let search_value = format!("{} {}", provider_id, provider_name.to_lowercase());
            let description = if requires_openai_auth {
                Some("Requires OpenAI login or API key.".to_string())
            } else {
                Some("Use an API key or bearer token.".to_string())
            };
        ...
        self.bottom_pane.show_selection_view(SelectionViewParams {
            ...
            header: self.model_menu_header(
                "Select Provider",
                "Choose a provider for /model and future turns.",
            ),
            items,
            is_searchable: true,
            ...
            footer_hint: Some(footer_hint),
            custom_hotkeys: vec![SelectionHotKeyAction {
                hotkey: SelectionHotKey::CtrlP,
                action: Box::new(|tx| {
                    tx.send(AppEvent::PromptAddCustomModelProvider);
                }),
                dismiss_on_trigger: true,
            }],
            ..Default::default()
    ...
    pub(crate) fn open_provider_api_key_prompt(
        ...
        let tx = self.app_event_tx.clone();
        let title = format!("{provider_name} API key");
        let placeholder = "Paste API key and press Enter".to_string();
        let context_label = if provider_id == "openai" {
            Some("Stored in auth.json for future sessions.".to_string())
        } else {
            Some("Stored in config.toml for future sessions.".to_string())
        };
        let view = CustomPromptView::new(
            title,
            placeholder,
    ...
    pub(crate) fn open_custom_provider_base_url_prompt(&mut self) {
        let tx = self.app_event_tx.clone();
        let view = CustomPromptView::new(
            "Add Custom Provider".to_string(),
            "Enter provider base URL (for example https://example.com/v1)".to_string(),
            None,
            Box::new(move |base_url: String| {
                tx.send(AppEvent::SubmitCustomProviderBaseUrl { base_url });
            }),
        );
        self.bottom_pane.show_view(Box::new(view));
    }

    pub(crate) fn open_custom_provider_api_key_prompt(&mut self, base_url: String) {
        let tx = self.app_event_tx.clone();
        let context_label = Some(format!("Base URL: {base_url}"));
        let view = CustomPromptView::new(
            "Custom Provider API key".to_string(),
            "Paste API key and press Enter".to_string(),
            context_label,
            Box::new(move |api_key: String| {
                tx.send(AppEvent::SubmitCustomProviderApiKey {
    ...
    pub(crate) fn open_openai_provider_auth_popup(&mut self) {
        self.bottom_pane.show_selection_view(SelectionViewParams {
            header: self.model_menu_header(
            ...
            items: vec![
                SelectionItem {
                    ...
                    description: Some("Use your ChatGPT account credentials.".to_string()),
                    actions: vec![Box::new(|tx| {
                        tx.send(AppEvent::UseOpenAiLogin);
                    })],
                    dismiss_on_select: true,
                    ..Default::default()
                },
                SelectionItem {
                    ...
                    description: Some(
                        "Store an OpenAI API key for usage-based billing.".to_string(),
                    ),
                    actions: vec![Box::new(|tx| {
                        tx.send(AppEvent::PromptProviderApiKey {
