...
use codex_core::OPENROUTER_PROVIDER_ID;
...
const MODEL_PICKER_BOOTSTRAP_MARKER: &str = "model_picker_bootstrap_seen";
...
pub(crate) struct App {
    ...
    model_picker_bootstrapped: bool,
...
impl App {
    ...
    pub async fn run(
        ...
        let model_picker_bootstrapped = config
            .codex_home
            .join(MODEL_PICKER_BOOTSTRAP_MARKER)
            .is_file();
    ...
    async fn handle_event(&mut self, tui: &mut tui::Tui, event: AppEvent) -> Result<AppRunControl> {
        match event {
            ...
            AppEvent::OpenModelPicker => {
                if !self.model_picker_bootstrapped {
                    self.model_picker_bootstrapped = true;
                    let marker_path = self.config.codex_home.join(MODEL_PICKER_BOOTSTRAP_MARKER);
                    if let Err(err) = std::fs::write(marker_path, b"1") {
                        tracing::warn!(error = %err, "failed to persist model picker bootstrap marker");
                    }
                    self.chat_widget.open_model_providers_popup(
                        self.visible_provider_items(),
                        self.config.model_provider_id.as_str(),
                    );
                    return Ok(AppRunControl::Continue);
                }
                self.open_model_picker_for_active_provider().await;
            }
            ...
            AppEvent::SelectModelProvider { provider_id } => {
                ...
                if provider.is_openai() && self.auth_manager.auth_cached().is_none() {
                    self.chat_widget.open_openai_provider_auth_popup();
                    return Ok(AppRunControl::Continue);
                }

                if Self::provider_needs_api_key(&provider) {
                    self.chat_widget
                        .open_provider_api_key_prompt(provider_id, provider.name.clone());
                    return Ok(AppRunControl::Continue);
                }

                self.switch_model_provider(provider_id, provider, None)
                    .await;
            }
            AppEvent::PromptProviderApiKey { provider_id } => {
                let Some(provider) = self.config.model_providers.get(provider_id.as_str()) else {
                    self.chat_widget
                        .add_error_message(format!("Unknown model provider: {provider_id}"));
                    return Ok(AppRunControl::Continue);
                };
            ...
            AppEvent::SubmitProviderApiKey {
                    ...
                    match login_with_api_key(
                        ...
                        Ok(()) => {
                            ...
                            let provider = self
                                ...
                                .get("openai")
                                .cloned()
                                .unwrap_or_else(|| built_in_model_providers()["openai"].clone());
                            self.switch_model_provider("openai".to_string(), provider, None)
                                .await;
                            self.chat_widget.add_info_message(
                                "Saved OpenAI API key and switched provider to OpenAI.".to_string(),
                                None,
                            );
                        }
                        Err(err) => {
                            self.chat_widget
                                .add_error_message(format!("Failed to save OpenAI API key: {err}"));
                        }
                    }
                    return Ok(AppRunControl::Continue);
                }

                let Some(base_provider) = self
                    .config
                    .model_providers
                    .get(provider_id.as_str())
                    .cloned()
                ...
                let mut provider_override = base_provider;
                provider_override.env_key = None;
                provider_override.env_key_instructions = None;
                provider_override.experimental_bearer_token = Some(trimmed_key.to_string());
                self.switch_model_provider(
                    provider_id,
                ...
                .await;
                self.chat_widget.add_info_message(
                    "Saved provider API key and refreshed model list.".to_string(),
                    None,
                );
            }
            AppEvent::PromptAddCustomModelProvider => {
                self.chat_widget.open_custom_provider_base_url_prompt();
            }
            AppEvent::SubmitCustomProviderBaseUrl { base_url } => {
                let Some(normalized_base_url) = Self::normalize_provider_base_url(&base_url) else {
                    self.chat_widget
                        .add_error_message("Provider base URL cannot be empty.".to_string());
                    return Ok(AppRunControl::Continue);
                };
                self.chat_widget
                    .open_custom_provider_api_key_prompt(normalized_base_url);
            }
            AppEvent::SubmitCustomProviderApiKey { base_url, api_key } => {
                let Some(normalized_base_url) = Self::normalize_provider_base_url(&base_url) else {
                    self.chat_widget
                        .add_error_message("Provider base URL cannot be empty.".to_string());
                    return Ok(AppRunControl::Continue);
                ...
                let provider_id = self.provider_id_from_base_url(&normalized_base_url);
                let provider_name = normalized_base_url
                    .split_once("://")
                    .map(|(_, rest)| rest)
                ...
                let provider = ModelProviderInfo {
                    ...
                    experimental_bearer_token: Some(trimmed_key.to_string()),
                ...
                self.switch_model_provider(provider_id, provider.clone(), Some(provider))
                    .await;
                self.chat_widget.add_info_message(
                    format!("Added custom provider `{provider_name}` and loaded available models."),
                    None,
                );
            }
            AppEvent::UseOpenAiLogin => {
                if self.config.model_provider_id != "openai"
                    && let Some(provider) = self.config.model_providers.get("openai").cloned()
                {
                    self.switch_model_provider("openai".to_string(), provider, None)
                        .await;
                }
                self.chat_widget.add_info_message(
                    "Run `codex login` to sign in with ChatGPT, then use /model again.".to_string(),
                    None,
                );
            }
    ...
    async fn open_model_picker_for_active_provider(&mut self) {
        let models = self.server.list_models(RefreshStrategy::Online).await;
        if models.is_empty() {
            if Self::provider_needs_api_key(&self.config.model_provider) {
                self.chat_widget.open_provider_api_key_prompt(
            ...
            self.chat_widget.add_error_message(format!(
                "No models were returned for provider `{}`.",
    ...
    fn visible_provider_items(&self) -> Vec<(String, String, bool)> {
        let built_in_ids = built_in_model_providers();
        let mut providers: Vec<(String, String, bool)> = self
            .config
            .model_providers
            .iter()
            .filter_map(|(provider_id, provider)| {
                let include = provider_id == "openai"
                    || provider_id == OPENROUTER_PROVIDER_ID
                    || provider_id == &self.config.model_provider_id
                    || !built_in_ids.contains_key(provider_id);
                include.then(|| {
                    (
                        provider_id.clone(),
                        provider.name.clone(),
            ...
            .collect();
        providers.sort_by(|(left_id, left_name, _), (right_id, right_name, _)| {
            let left_rank = Self::provider_sort_rank(left_id);
            let right_rank = Self::provider_sort_rank(right_id);
            left_rank
                .cmp(&right_rank)
    ...
    fn provider_sort_rank(provider_id: &str) -> i32 {
        match provider_id {
            "openai" => 0,
            OPENROUTER_PROVIDER_ID => 1,
            _ => 10,
        }
    }

    fn provider_needs_api_key(provider: &ModelProviderInfo) -> bool {
        ...
        matches!(provider.api_key(), Err(_))
    }

    fn model_provider_segments(active_profile: Option<&str>) -> Vec<String> {
    ...
    fn provider_override_edits(provider_id: &str, provider: &ModelProviderInfo) -> Vec<ConfigEdit> {
        ...
        edits.push(ConfigEdit::SetPath {
            ...
            value: value(provider.name.clone()),
        });
    ...
    fn normalize_provider_base_url(input: &str) -> Option<String> {
    ...
    fn provider_id_from_base_url(&self, base_url: &str) -> String {
        ...
        let base = format!("custom-{host}");
        if !self.config.model_providers.contains_key(base.as_str()) {
            return base;
        }
        let mut index = 2;
        loop {
            let candidate = format!("{base}-{index}");
            if !self.config.model_providers.contains_key(candidate.as_str()) {
                return candidate;
            }
            index += 1;
        }
    ...
    async fn switch_model_provider(
        ...
        let mut edits = vec![ConfigEdit::SetPath {
            segments: Self::model_provider_segments(self.active_profile.as_deref()),
            value: value(provider_id.clone()),
        }];
        if let Some(override_provider) = provider_override.as_ref() {
            edits.extend(Self::provider_override_edits(
                provider_id.as_str(),
                override_provider,
            ));
        }
        ...
        {
            self.chat_widget
                .add_error_message(format!("Failed to save provider selection: {err}"));
            return;
        }
        ...
        if let Some(override_provider) = provider_override {
            self.config
                .model_providers
                .insert(provider_id.clone(), override_provider);
        }
        self.config.model_provider_id = provider_id.clone();
        self.config.model_provider = provider.clone();
        self.chat_widget
            .set_model_provider(provider_id.clone(), provider.clone());
        self.server
            .set_models_provider(provider_id.clone(), provider)
            .await;
        let models = self.server.list_models(RefreshStrategy::Online).await;
        if models.is_empty() {
            self.chat_widget.add_error_message(format!(
                "No models were returned for provider `{provider_id}`."
            ));
            return;
        }

        let current_model = self.chat_widget.current_model().to_string();
        let selected_preset = models
            .iter()
        ...
        if let Some(preset) = selected_preset {
            self.chat_widget.set_model(preset.model.as_str());
            self.on_update_reasoning_effort(Some(preset.default_reasoning_effort));
            self.app_event_tx
                .send(AppEvent::CodexOp(Op::OverrideTurnContext {
                    cwd: None,
                    approval_policy: None,
                    ...
                    effort: Some(Some(preset.default_reasoning_effort)),
                    summary: None,
                    service_tier: None,
                    collaboration_mode: None,
                    personality: None,
                    provider: serde_json::to_value(&self.config.model_provider).ok(),
                }));
            self.app_event_tx.send(AppEvent::PersistModelSelection {
                model: preset.model.clone(),
                effort: Some(preset.default_reasoning_effort),
            });
        }
...
mod tests {
    ...
    async fn make_test_app() -> App {
        ...
        App {
            ...
            auth_manager,
            config,
            model_picker_bootstrapped: true,
            active_profile: None,
